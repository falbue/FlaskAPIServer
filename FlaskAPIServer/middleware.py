...
logger = logger_setup("MIDDLEWARE", config.DEBUG, log_path=config.LOG_PATH)
...
def _refresh_api_keys_cache():
    ...
    try:
        ...
        if result:
            _api_keys_cache = {
                item["key"]: {"role": item["role"], "priority": item["priority"]}
                for item in result
            }
            _cache_last_updated = time.time()
            logger.info(
                f"Обновлен кеш API-ключей. Загружено ключей: {len(_api_keys_cache)}"
            )
        else:
        ...
        roles_result = SQL_request(
            "SELECT name, priority FROM key_roles ORDER BY priority DESC", fetch="all"
        )
        if roles_result:
            _roles_hierarchy_cache = {
                role["name"]: role["priority"] for role in roles_result
            }
...
def _get_jwt_role_info(role_name):
    ...
    if role_name in _roles_hierarchy_cache:
        return {"role": role_name, "priority": _roles_hierarchy_cache[role_name]}
    return None
