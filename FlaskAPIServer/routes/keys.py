...
from .. import config
from ..middleware import generate_jwt_token, key_role, refresh_api_keys
from ..utils.database import SQL_request

logger = logger_setup("API_KEYS", config.DEBUG, log_path=config.LOG_PATH)
PREFIX_KEYS = "/api/keys"

# Blueprint for API keys routes
keys = Blueprint("keys", __name__)


@keys.route(f"{PREFIX_KEYS}", methods=["GET"])
@key_role("api_key")
def get_all_keys():
    try:
        keys = SQL_request(
...
@keys.route(f"{PREFIX_KEYS}", methods=["POST"])
@key_role("api_key")
def api_create_key():
    try:
...
@keys.route(f"{PREFIX_KEYS}/<key>", methods=["PATCH"])
@key_role("api_key")
def update_key(key):
    try:
        data = request.get_json()
...
@keys.route(f"{PREFIX_KEYS}/<key>", methods=["DELETE"])
@key_role("api_key")
def delete_key(key):
    try:
        SQL_request("DELETE FROM api_keys WHERE key = ?", (key,), fetch=None)
...
@keys.route(f"{PREFIX_KEYS}/refresh", methods=["GET"])
@key_role("api_key")
def refresh_keys():
    try:
        refresh_api_keys()
...
@keys.route(f"{PREFIX_KEYS}/jwt", methods=["GET", "POST"])
def generate_token():
    try:
